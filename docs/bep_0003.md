...
20 字节长度的 sha1 哈希，对种子文件的 `info` 部分进行哈希生成。在HTTP
query中需要[进行转义](https://zh.wikipedia.org/wiki/%E7%99%BE%E5%88%86%E5%8F%B7%E7%BC%96%E7%A0%81)。

由于bencode的特，可以先解析整个种子文件，对 `info` 部分进行bencode编码，
然后再计算对应 sha1 哈希。
...
!!! tip
    ...
    以 [ubuntu-24.04-live-server-amd64](/ubuntu-24.04-live-server-amd64.iso.torrent) 为例，他的 info_hash 的 hex 编码
    为 `81af07491915415dad45f87c0c2ae52fae92c06b`，

    这里实际上是 `hashlib.sha1(...).digest()` 的结果，
    但是由于二进制内容不一定能够对应可显示的内容，所以一般以hex进行编码。
...
`ip` 为字符串，表示其他peer的ip地址或者dns name。`port` 则为数字。`peer id` 见前文。

peer
也有可能请求紧凑（compact）的响应，在[bep 0023](https://www.bittorrent.org/beps/bep_0023.html)
中进行定义。
